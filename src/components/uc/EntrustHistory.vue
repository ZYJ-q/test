...
<template>
  <div class="entrusthistory">
    <Form class="form" :model="formItem" :label-width="60" inline>
      <FormItem
        :label-width="locale == 'en' ? 144 : 60"
        :label="$t('historyAndCu.stEnTime')"
      >
        <DatePicker
          type="daterange"
  ...
  computed: {
    columns () {
      ...
      arr.push({
        ...
        key: "triggerPrice",
        render: (h, params) => {
          return h('span', {}, params.row.triggerPrice ? params.row.triggerPrice : 0)
        }
      })
      arr.push({
      ...
      //                 }
      //             },
      //             this.toFloor(params.row.price+'111')
      //         );
      //     }
      // });
      arr.push({
