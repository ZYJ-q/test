...
export default {
    ...
    methods: {
        init() {
            ...
            //     this.data = [...this.leverList, ...this.otcList, ...this.exchangeList,...this.hyList]
            // })
            Promise.all([this.getOtcNum(), this.getExchangeList(),this.getHyNum()]).then(res => {
                this.data = [...this.leverList, ...this.otcList, ...this.exchangeList,...this.hyList,...this.BBWList]
            })
        },
        cancel() {
            this.value = [];
        ...
        //获取合约账户
        getHyNum() {
            return this.$http.post(this.host + "/contract-api/contract-wallet/get").then(response => {//查询法币种类的个数
                var resp = response.body;
                if (resp.code == 0) {
                    const temp = resp.data.length > 0 && resp.data.map(ele => {
                        return {
		...
		//获取币本位账户
		getBBW() {
		    return this.$http.post(this.host + "/contract-coin-api/contract-wallet/get").then(response => {//查询法币种类的个数
		        var resp = response.body;
				console.log('币本位',response)
		        if (resp.code == 0) {
		            const temp = resp.data.length > 0 && resp.data.map(ele => {
		                return {
		                    value: ele.coinId,
        ...
        getValue(value, selectedData) {
            if (value.length > 0) {
                ...
                if (str === this.$t('coin.Leveraged')) {
                    ...
                    const list2={
                        value: this.$t('coin.contract'),
                        label: this.$t('coin.contract')
                    }

                    this.toData = [...this.leverList, list,list2];
                    console.log(this.toData);
                }else if(str == this.$t('coin.contract')){
                    const list = {
                        value: this.$t('coin.bit'),
                        label: this.$t('coin.bit')
                    }
                    this.toData = [list];
                }else if(str == this.$t('coin.bibenwei')){
					const list = {
					    value: this.$t('coin.bit'),
					    label: this.$t('coin.bit')
					}
        ...
        sure() {
            ...
            if (this.value[0] == this.$t('coin.bit') || this.value[0] == this.$t('coin.legal')) {
                ...
                //币币账户转合约账户
                if (this.toValue[0] == this.$t('coin.contract')) {
                    const params = {
                        coinName: this.value[1],
                        amount: this.num,
                        direction: 2
				...
				//币币账户转币本位账户
				if (this.toValue[0] == this.$t('coin.bibenwei')) {
				    const params = {
				        coinName: this.value[1],
				        amount: this.num,
				        direction: 4
            ...
            // 合约账户转币币账户
            if(this.value[0] == this.$t('coin.contract')){
                if (this.toValue[0] == this.$t('coin.bit')) {
                    const params = {
                        coinName: this.value[1],
                        amount: this.num,
			...
			// 币本位账户转币币账户
			if(this.value[0] == this.$t('coin.bibenwei')){
			    if (this.toValue[0] == this.$t('coin.bit')) {
			        const params = {
			            coinName: this.value[1],
			            amount: this.num,
