      ...
      <div class="item">
        ...
        <Dropdown @on-click="changeContract">
          <div style="display: flex">
            <a href="javascript:void(0)" style="color: rgb(240, 167, 10)">
              <span class="coin">
                {{ contractText }}
                <small>/USDT</small>
              </span>
              <Icon type="ios-arrow-down" style="color: #f0a70a"></Icon>
            </a>
          </div>
                ...
                <ul>
                  <template v-for="(item, index) in methodsList">
                    <li
                      ...
                      :class="{
                        active: item.check,
                        hide2: btnList[1].check == true && index == 2,
                      }"
                    >
                      {{ item.text }}
                    </li>
                    <!--                    <li @click="tabMN(index)" :class="{active:item.check,hide2:index==2}">{{item.text}}</li>-->
                  </template>
                </ul>
              </div>
...
export default {
  ...
  watch: {
    ...
    $route (to, from) {
      ...
      this.contractText = params.toUpperCase().split("_")[0]
    },
    // contractText:function(val){
    //   console.log(val);
    //   this.init();
    // },
  ...
  methods: {
      ...
      this.$http.post(this.host + this.api.contract.thumb, {}).then(response => {
        ...
        for (let i = 0; i < resp.length; i++) {
          ...
          if (coin.symbol == this.currentCoin.coinSymbol2) {
            ...
            console.log(this.currentCoin.coinSymbol2)
    ...
    getCurrentOrder: function (type) {
      ...
      let params = {}
      params["memberId"] = this.member.id
      params["symbol"] = route ? route.split('_')[0] + route.split('_')[1] : ''
      params["pageNo"] = 1
