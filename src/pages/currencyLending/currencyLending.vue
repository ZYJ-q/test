  ...
  <div class="content">
    ...
    <div class="BuyandSell">
      <div
        ...
        class="flex items_center column"
      >
        <!-- 借入市场和借出市场切换 -->
        <Menu mode="horizontal" class="Menu1" :theme="theme1" active-key="1">
          ...
          <Menu-item key="1" :class="show_borrow ? 'lend' : 'li1'">
            <span style="padding: 20px 0" @click="isBorrow">{{
              $t("currencyLending").jrsc
            }}</span>
          </Menu-item>
      ...
      <div class="cengji">
        ...
        <div>
          <!-- 开启 -->
          <span class="mr20 yellow_hover pointer" @click="releaseBorrow">{{
            $t("currencyLending").fbjrd
          }}</span>
          <span class="yellow_hover pointer" @click="releaseLend">{{
            $t("currencyLending").fbjcd
          }}</span>
          <!-- 暂未开放 -->
        ...
        <div>
          ...
          <Select
            ...
            style="width: 100px"
          >
            <Option v-for="(item, i) in cycle" :key="i" :value="item.value">{{
              item.label
            }}</Option>
          </Select>
        </div>
      ...
      <div v-if="!show_borrow">
          ...
          <div class="list_">
            ...
            <div class="flex items_center">
              <div class="flex cloumn">
                <p class="gray font14">
                  {{ $t("currencyLending").kjrje }}&nbsp;&nbsp;({{
                    $t("currencyLending").zs
                  }}￥{{ v.minAmount }})
                </p>
              ...
              <div class="flex cloumn">
                ...
                <p class="bold font18 block">
                  {{ v.period }}{{ $t("currencyLending").t }}
                </p>
              </div>
              <i-button
                v-if="v.status == 0"
                ...
                "
                >{{ $t("currencyLending").jc }}</i-button
              >
              <i-button v-if="v.status == 1" disabled type="warning">{{
                $t("currencyLending").ywc
              }}</i-button>
              <i-button v-if="v.status == 2" disabled type="warning">{{
                $t("currencyLending").ysw
              }}</i-button>
        ...
        <div
          v-if="borrow.length <= 0"
          style="width: 100%; height: 200px"
          class="wAll flex flex-align-center flex-pack-center noIeoTip"
        >
          ...
          <div class="list_">
            ...
            <div class="flex items_center">
              ...
              <i-button
                ...
                "
                >{{ $t("currencyLending").jr }}</i-button
              >
              <i-button v-if="v.status == 1" type="warning" disabled>{{
                $t("currencyLending").ywc
              }}</i-button>
              <i-button v-if="v.status == 2" type="warning" disabled>{{
                $t("currencyLending").ysw
              }}</i-button>
              <!-- <div style='height: 40px;background: #f5c125;'>
								<i-button type="warning">警告按钮</i-button>
    ...
    <div class="zhezhao_" v-if="showModal_">
      <div class="modal_">
        <p class="flex between items_center">
          <span class="font20 block ml8">{{
            $t("currencyLending").txjrd
          }}</span>
        ...
        <div class="kuang_">
          ...
          <div class="flex lineH40">
            ...
            <i-select
              :model.sync="borrowForm.choose_coin"
              @on-change="borrowForm.coinHS"
              :placeholder="borrowForm.choose_coin"
              style="width: 200px"
            >
              <i-option
                v-for="(item, i) in borrowForm.coin"
                :key="i"
                :value="item.value"
                >{{ item.label }}</i-option
              >
          ...
          <div class="flex lineH40">
            ...
            <Input-number
              ...
              :max="100000"
              :min="1"
              @on-change="borrowForm.moneyHS"
              @on-blur="borrowForm.moneyBlur"
          ...
          <div class="flex lineH40">
            ...
            <i-select
              :model.sync="borrowForm.day"
              @on-change="borrowForm.zhouqiHS"
              :placeholder="borrowForm.day"
              style="width: 200px"
            >
              <i-option
                v-for="(item, i) in borrowForm.zhouqi"
                :key="i"
                :value="item.label"
                >{{ item.label }}</i-option
              >
          ...
          <div class="flex lineH40">
            ...
            <Input-number
              ...
              :step="0.001"
              :min="0.01"
              @on-change="borrowForm.rililvHS"
              :value="borrowForm.rililv"
            ></Input-number>
            <div class="qianhui pd_lr6">
              {{ $t("currencyLending").zhnh }}：{{ borrowForm_zhehenianhua }}%
            </div>
          </div>
          <div class="flex lineH40">
          ...
          <div class="flex lineH40">
            <div class="w165 txt_right">
              {{ $t("currencyLending").dbzxjkje }}：
            </div>
            <Input-number
              style="width: 310px"
              :max="10000"
              :min="1000"
              @on-change="borrowForm.small_numHS"
              :value="borrowForm.small_num"
            ></Input-number>
            <div class="qianhui pd_lr6">USDT</div>
          </div>
          <div class="flex lineH40">
            <div class="w165 txt_right">
              {{ $t("currencyLending").dbzdjkje }}：
            </div>
            <Input-number
              style="width: 310px"
              :max="10000"
              :min="1000"
              @on-change="borrowForm.max_numHS"
              :value="borrowForm.max_num"
            ></Input-number>
            <div class="qianhui pd_lr6">USDT</div>
          </div>
          <div class="flex lineH40">
          ...
          <div class="flex lineH40">
            ...
            <i-input
              v-model="jy_password"
              type="password"
              :placeholder="$t('currencyLending').qsrjymm"
              style="width: 310px"
            ></i-input>
          </div>
          <div class="flex jst_center mt30 lineH40">
            <i-button
              type="warning"
              :disabled="disabled_"
              style="margin-right: 26px; width: 165px"
              @click="tijiao(1)"
              >{{ $t("currencyLending").tj }}</i-button
            >
            <!-- <Checkbox :checked.sync="single">我已阅读并同意<span style="margin-left: 4px;">《QKL法币借贷协议》</span></Checkbox> -->
            <i-button class="gray" style="width: 165px" @click="close()">{{
              $t("currencyLending").gb
            }}</i-button>
          </div>
        </div>
      </div>
      ...
      <div class="modal_" style="height: 530px">
        ...
        <div class="kuang_">
          ...
          <div class="flex lineH50">
            <div class="w165 txt_right">{{ $t("currencyLending").jkzq }}：</div>
            <i-select
              :model.sync="LendForm.day"
              @on-change="LendForm.zhouqiHS"
              :placeholder="LendForm.day"
              style="width: 200px"
            >
              <i-option
                v-for="(item, i) in LendForm.zhouqi"
                :key="i"
                :value="item.label"
                >{{ item.label }}</i-option
              >
          ...
          <div class="flex lineH50">
            ...
            <Input-number
              ...
              :step="0.001"
              :min="0.01"
              @on-change="LendForm.rililvHS"
              :value="LendForm.rililv"
            ></Input-number>
            <div class="qianhui pd_lr6">
              {{ $t("currencyLending").zhnh }}：{{ lendForm_zhehenianhua }}%
            </div>
          </div>
          <div class="flex lineH50">
          ...
          <div class="flex lineH50">
            ...
            <Input-number
              ...
              :max="10000"
              :min="1000"
              @on-change="LendForm.small_numHS"
              :value="LendForm.small_num"
            ></Input-number>
            <div class="qianhui pd_lr6">USDT</div>
          </div>
          <div class="flex lineH50">
            ...
            <Input-number
              ...
              :max="10000"
              :min="1000"
              @on-change="LendForm.max_numHS"
              :value="LendForm.max_num"
            ></Input-number>
            <div class="qianhui pd_lr6">USDT</div>
          </div>
          <div class="flex lineH40">
          ...
          <div class="flex jst_center mt30 lineH40">
            ...
            >
            <!-- <Checkbox :checked.sync="single">我已阅读并同意<span style="margin-left: 4px;">《QKL法币借贷协议》</span></Checkbox> -->
            <i-button style="width: 165px" class="gray" @click="close()">{{
              $t("currencyLending").gb
            }}</i-button>
          </div>
        </div>
    ...
    <Modal v-model="dianjijieru" title="借入" width="630">
      <p slot="header" style="color: rgba(240, 185, 11, 1); text-align: center">
        <span style="padding-left: 5px"
          >{{ $t("currencyLending").x }}{{ jieru_tianxie.name
          }}{{ $t("currencyLending").jr }}USDT</span
        >
      </p>
      <p>
      ...
      <div>
        <Input-number
          ...
          :step="100"
          :min="100"
          @on-change="jieru_tianxie.moneyHS"
          :value="jieru_tianxie.money"
        ></Input-number>
      </div>
      <p class="mt10">
      ...
      <div>
        <Select
          v-model="jieru_tianxie.diya_currency.diya"
          :placeholder="jieru_tianxie.diya_currency.diya"
          @on-change="jieru_tianxie.chooseType"
          style="width: 200px"
        >
          <Option
            v-for="(v, i) in jieru_tianxie.diya_currency.coin_type"
            :value="v.name"
            :key="i"
            >{{ v.name }}</Option
          >
      ...
      <div class="four_bu">
        ...
        <p class="flex column">
          ...
          <span class="bold font16"
            >{{ jieru_tianxie.day }}{{ $t("currencyLending").t }}</span
          >
        </p>
      </div>
      <div class="flex">
        ...
        <p class="ml40">
          ...
          <span style="color: rgba(240, 185, 11, 1)" class="bold font20">{{
            jieru_tianxie_fee
          }}</span>
        </p>
      </div>
      <div class="mt10">
        <Input
          type="password"
          v-model="jieru_tianxie.password"
          :placeholder="$t('currencyLending').qsrjymm"
          style="width: 300px"
        />
      </div>
      ...
      >
        <div>
          <Button type="text" @click="jieru_tianxie.quxiao">{{
            $t("currencyLending").qx
          }}</Button>
          <Button type="warning" @click="jieru_tianxie.queding">{{
            $t("currencyLending").qd
          }}</Button>
        </div>
      </div>
    </Modal>
    ...
    <Modal v-model="dianjijiechu" title="借出" width="630">
      <p slot="header" style="color: rgba(240, 185, 11, 1); text-align: center">
        <span style="padding-left: 5px"
          >{{ $t("currencyLending").jc }}USDT{{ $t("currencyLending").g
          }}{{ jiechu_tianxie.name }}</span
        >
      </p>
      <p>
      ...
      <div>
        <Input-number
          ...
          :step="100"
          :min="100"
          @on-change="jiechu_tianxie.moneyHS"
          :value="jiechu_tianxie.money"
        ></Input-number>
      </div>
      <div class="four_bu">
        <p class="flex column">
        ...
        <p class="flex column">
          <span>{{ $t("currencyLending").jkzq }}</span>
          <span class="bold font16"
            >{{ jiechu_tianxie.day }}{{ $t("currencyLending").t }}</span
          >
        </p>
      </div>
      <div class="flex">
      ...
      <div class="mt10">
        <Input
          type="password"
          v-model="jiechu_tianxie.password"
          :placeholder="$t('currencyLending').qsrjymm"
          style="width: 300px"
        />
      </div>
      ...
      >
        <div>
          <Button type="text" @click="jiechu_tianxie.quxiao">{{
            $t("currencyLending").qx
          }}</Button>
          <Button class="gray" @click="jiechu_tianxie.queding">{{
            $t("currencyLending").qd
          }}</Button>
        </div>
      </div>
