      ...
      <div class="right">
        <div class="coin-menu">
          <div class="sc_filter">
            ...
            >
            <span
              @click="changeBaseCion('btc')"
              :class="{ active: basecion === 'btc' }"
              >BTC</span
            >
            <span
              @click="changeBaseCion('eth')"
              :class="{ active: basecion === 'eth' }"
              >ETH</span
            >
            <span
              v-show="isLogin"
              @click="changeBaseCion('favor')"
              :class="{ active: basecion === 'favor' }"
              >{{ $t("coin.option") }}</span
            >
            ...
            <div class="mask" v-show="!isLogin">
              <span
                ...
                <router-link to="/login">
                  <span style="color: rgb(240, 167, 10)">{{
                    $t("common.login")
                  }}</span>
                </router-link>
                /
                <router-link to="/register">
                  <span style="color: rgb(10, 220, 255)">{{
                    $t("common.register")
                  }}</span>
                </router-link>
              </span>
            ...
            <div class="trade_bd">
              <div class="panel panel_buy">
                <div v-if="isLogin" class="hd hd_login">
                  ...
                  <router-link :to="rechargeUSDTUrl">{{
                    $t("exchange.recharge")
                  }}</router-link>
                  ...
                  <Form ref="formValidate">
                    <FormItem>
                      ...
                      <p class="math_price">
                        ≈
                        {{
                          (currentCoin.usdRate / currentCoin.close) *
                            form.buy.limitPrice *
                            CNYRate || 0 | toFixed(2)
                        }}
                        CNY
                    ...
                    <FormItem>
                      ...
                      <label
                        ><span>{{ $t("exchange.bestbuy") }}:</span
                        >{{
                          (this.wallet.base / this.form.buy.limitPrice).toFixed(
                            4
                          )
                        }}</label
                      >
                ...
                <div class="bd bd_limited" v-show="btnList[1].check == true">
                  <Form ref="formValidate" :label-width="50">
                    <FormItem :label="$t('coin.trigger')">
                      <Input
                        @on-keyup="keyEvent"
                        v-model="form.buy.stopPrice"
                        placeholder=""
                      ></Input>
                      <label>{{ currentCoin.base }}</label>
                    ...
                    <FormItem :label="$t('coin.volume')">
                      <Input
                        @on-keyup="keyEvent"
                        v-model="form.buy.stopBuyAmount"
                        :placeholder="$t('exchange.buynum')"
                      ></Input>
                      <label>{{ currentCoin.coin }}</label>
                    </FormItem>
                    <div class="slider-wrap">
                      ...
                      <div
                        ...
                        v-for="item in sliderStep"
                        :style="'left: ' + item + '%;'"
                        @click="silderGo('sliderBuyStopPercent', item)"
                      >
                        <div class="slider-block"></div>
                      </div>
                    </div>
                    <div class="total buy_total">
                      {{ $t("exchange.amount") }}
                      <span>{{
                        form.buy.stopTurnover | toFloor(baseCoinScale)
                      }}</span>
                      {{ currentCoin.base }}
                    </div>
                    <Button
                ...
                <div class="bd bd_market" style="display: none">
                  <Form ref="formValidate">
                    <FormItem>
                      <Input
                        disabled
              ...
              <div class="panel panel_sell">
                <div v-if="isLogin" class="hd hd_login">
                  ...
                  <router-link :to="rechargeCoinUrl">{{
                    $t("exchange.recharge")
                  }}</router-link>
                ...
                <div class="bd bd_limited" v-show="btnList[1].check == true">
                  <Form ref="formValidate" :label-width="50">
                    <FormItem :label="$t('coin.trigger')">
                      <Input
                        @on-keyup="keyEvent"
                        v-model="form.sell.stopPrice"
                        :placeholder="$t('coin.trigger')"
                      ></Input>
                      <label>{{ currentCoin.base }}</label>
                    ...
                    <FormItem :label="$t('coin.sellingRate')">
                      <Input
                        @on-keyup="keyEvent"
                        v-model="form.sell.stopBuyPrice"
                        :placeholder="$t('exchange.sellnum')"
                      ></Input>
                      <label>{{ currentCoin.base }}</label>
                    </FormItem>
                    <FormItem :label="$t('coin.sellingVolume')">
                      <Input
                        @on-keyup="keyEvent"
                        v-model="form.sell.stopBuyAmount"
                        :placeholder="$t('exchange.sellnum')"
                      ></Input>
                      <label>{{ currentCoin.coin }}</label>
                    </FormItem>
                    ...
                    <div class="slider-wrap">
                      ...
                      <div
                        ...
                        v-for="item in sliderStep"
                        :style="'left: ' + item + '%;'"
                        @click="silderGo('sliderSellStopPercent', item)"
                      >
                        <div class="slider-block"></div>
                      </div>
                    </div>
                    ...
                    <div class="total sell_total">
                      {{ $t("exchange.amount") }}
                      <span>{{
                        form.sell.stopTurnover | toFloor(coinScale)
                      }}</span>
                      {{ currentCoin.base }}
                    </div>
                    <Button
      ...
      .trade_wrap {
        ...
        .trade_panel {
          ...
          .mask {
            ...
            height: 100%;
            display: flex;
            background-color: rgba(40, 49, 60, 0.6);
            justify-content: center;
            align-items: center;
            z-index: 100;
            font-size: 24px;
...
const map = new Map([['LIMIT_PRICE', '限价'], ['MARKET_PRICE', '市价'], ['CHECK_FULL_STOP', '计划委托']])
...
export default {
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              if (params.row.amount) {
                width = (params.row.amount / totle).toFixed(4) * 100 + "%"
              }
              return h(
                "div",
                {
      ...
      currentOrder: {
        columns: [
          ...
          },
          {
            ...
            key: "totalMoney",
            render (h, params) {
              return h("span", self.toFloor(params.row.totalMoney))
            }
          },
          {
  ...
  methods: {
    ...
    init () {
      ...
      let coin = params.toUpperCase().split("_")[0]//前
      let base = params.toUpperCase().split("_")[1]//后
    ...
    updateLangData () {
      ...
      this.currentOrder.columns[3].title = this.$t("coin.type")
      ...
      this.currentOrder.columns[8].title = this.$t("coin.amount")
      ...
      this.btnList[1].text = this.$t("contract.jhwt")
      ...
      this.$http.post(this.host + this.api.market.thumb, {}).then(response => {
        ...
        for (let i = 0; i < resp.length; i++) {
          ...
          if (coin.symbol == this.currentCoin.symbol) {
            ...
            this.form.buy.stopBuyPrice = this.form.sell.stopBuyPrice = coin.price
          }
        }
        if (this.isLogin) {
          this.getFavor()
    ...
    successCallBack (that) {
      ...
      //     stompClient.send("/app/message/hoding", {}, JSON.stringify({msg: 'holding'}));
      // }, 60000);
      ...
      stompClient.subscribe(
        ...
        function (msg) {
          ...
          if (resp.direction == "SELL") {
            let asks = resp.items
            asksLet = asks
            // if(asksLet==null||(bidsLet!=null&&bidsLet.length>0&&asksLet[0].price<bidsLet[0].price)){
            //     return;
            // }
            that.plate.askRows = []
            let totle = 0
            ...
            let bids = resp.items
            bidsLet = bids
            // if(bidsLet==null||(asksLet!=null&&asksLet.length>0&&asksLet[0].price<bidsLet[0].price)){
            //     return;
            // }
            that.plate.bidRows = []
            let totle = 0
      ...
      //订阅盘口消息
      stompClient.subscribe(
        "/topic/market/trade-robot-plate/" + that.currentCoin.symbol,
        function (msg) {
          let resp = JSON.parse(msg.body)
          let asks = resp.sell.items
          let bids = resp.buy.items
          that.plate.askRows = []
          that.plate.bidRows = []
          let totle = 0
          for (let j = 0, i = that.plate.maxPostion - 1; j < that.plate.maxPostion, i >= 0; j++, i--) {
            let ask = {}
            let bid = {}
            if (i < asks.length) {
              ask = asks[i]
    ...
    startWebsock () {
      ...
      stompClient.connect({}, (frame) => {
        ...
        this.reconnect(that.host + that.api.market.ws, this.successCallBack, that, this.isSelfClose)
      })
    },
