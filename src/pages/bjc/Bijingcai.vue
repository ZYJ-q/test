      ...
      <div class="bjc">
        <ul class="bjclist">
          <li v-for="(item, index) in bjclist" :key="index">
            ...
            <div class="content">
              <div class="symbol">
                1{{ item.quizType.quizCurrency }} / {{ item.quizType.betAmount
                }}{{ item.quizType.betCurrency }}
              </div>
              <child
                :content="item.quizType.deadline"
                @tozero="toZero(index)"
              ></child>
            ...
            <div class="circle">
              <i-circle
                :percent="(item.betNum / item.maxSum) * 100"
                dashboard
                :size="200"
                stroke-color="#ffa020"
                trail-color="#e6e6e6"
            ...
            <div v-if="item.maxSum - item.betNum == 0" class="button">
              <Button
                type="warning"
                @click="order(item.periodNo, item.quizType.id)"
                :disabled="true"
                >立即下注</Button
              >
            </div>
            <div v-else class="button">
              <Button
                type="warning"
                @click="order(item.periodNo, item.quizType.id)"
                :disabled="bjclist[index].disabled"
                >立即下注</Button
              >
            </div>
          </li>
